---
layout: workshop
year: 2025
date: Monday, April 28th, 2025
location: ICLR 2025 in Singapore
openreview: https://openreview.net/group?id=ICLR.cc/2025/Workshop/Re-Align
show_cfp: true

cover_image:
  image: "/assets/images/locations/singapore.jpg"
  author: Brandon Lim
  licence_name: "CC BY-SA 2.0"
  licence_link: "https://creativecommons.org/licenses/by-sa/2.0/"

buttons:
  - text: OpenReview
    url: https://openreview.net/group?id=ICLR.cc/2025/Workshop/Re-Align
  - text: ICLR.cc
    url: https://iclr.cc/virtual/2025/workshop/23963

cfp:
    dates:
      - event: Paper submission deadline
        date: <strike>Monday, February 3rd, 2025</strike> <br> Wednesday, February 5th, 2025
      - event: Reviewing deadline
        date: Friday, February 21st, 2025
      - event: Author notification
        date: Wednesday, March 5th, 2025
      - event: Camera-ready copy (CRC) deadline
        date: Sunday, April 20th, 2025
    extra_instructions: |
        ##### Funding for contributors to ICLR 2025
        This year, ICLR is discontinuing the separate "Tiny Papers" track, and 
        is instead requiring each workshop to accept short paper 
        submissions, with an eye toward inclusion; see the
        [ICLR page on Tiny Papers](https://iclr.cc/Conferences/2025/CallForTinyPapers) 
        for more details. Authors of these papers will be earmarked for 
        potential funding from ICLR, but need to submit a separate application 
        for Financial Assistance that evaluates their eligibility. This 
        application for Financial Assistance to attend ICLR 2025 is available at
        [ICLR 2025 Financial Assistance](https://iclr.cc/Conferences/2025/FinancialAssistance),
        and closes on March 2nd.


speakers:
  - name: Iris Groen
    affiliation: University of Amsterdam
    website: https://sites.google.com/view/irisgroen
    image: profile_iris.png
  - name: Dianbo Liu
    affiliation: National University of Singapore
    website: https://www.cogai4sci.com/
    image: profile_liu.jpg
  - name: Martin Schrimpf
    affiliation: EPFL
    website: https://mschrimpf.altervista.org/
    image: profile_schimpf.jpg
  - name: Janet Wiles
    affiliation: University of Queensland
    website: https://eecs.uq.edu.au/profile/2444/janet-wiles
    image: profile_wiles.jpg
  - name: Alex Williams
    affiliation: NYU
    website: https://neurostatslab.org/
    image: profile_williams.jpg

organizers:
  - name: Brian Cheung
    affiliation: MIT
    website: https://briancheung.github.io/
    image: brian.jpg
  - name: Dota Dong
    affiliation: MPI for Psycholinguistics
    website: https://tianaidong.github.io/
    image: dota.jpg
  - name: Erin Grant
    affiliation: UCL
    website: https://eringrant.github.io/
    image: erin.jpeg
  - name: Ilia Sucholutsky
    affiliation: NYU
    website: https://ilia10000.github.io/
    image: ilia.jpeg
  - name: Lukas Muttenthaler
    affiliation: TU Berlin
    website: https://lukasmut.github.io/
    image: lukas.jpeg
  - name: Siddharth Suresh
    affiliation: University of Wisconsin-Madison
    website: https://www.sidsuresh.com/
    image: sid.jpg

news:
  - content: All decisions have been sent out! The list of accepted papers can be found on <a href="https://openreview.net/group?id=ICLR.cc/2025/Workshop/Re-Align">OpenReview</a>. The workshop schedule can be found on the <a href="https://iclr.cc/virtual/2025/workshop/23963">ICLR.cc workshop site</a>. We are super excited to see you all in Singapore!
  - content: Small deadline extension! The deadline for for contributed papers is now <b>Wednesday, February 5th, 2025</b>.
  - content: "Announcing the workshop date: <b>Monday, April 28th, 2025</b> (Day 2 of the ICLR Workshops)!"
  - content: The <a href="#cfp">call for contributed papers</a> at Re-Align 2025 is now live!
  - content: The 2025 edition of Re-Align was accepted as a workshop at <a href="https://iclr.cc/Conferences/2025/">ICLR 2025</a>!

about: |
  Both natural and artificial intelligences form representations of the world that they use to reason, make decisions,
  and communicate. Despite extensive research across machine learning, neuroscience, and cognitive science, it
  remains unclear what the most appropriate ways are to compare and align the representations of intelligent systems
  ([Sucholutsky et al., 2023](https://arxiv.org/abs/2310.13018)). 
  In the second edition of the Workshop on Representational Alignment (Re-Align), we
  bring together researchers from diverse fields who study representational alignment to make concrete progress on
  this set of open interdisciplinary problems. We invite researchers across the machine learning, neuroscience, and
  cognitive science communities to participate in the workshop, and to contribute to the workshop in two ways:

  **First, in the form of [contributed papers](#cfp)** that
  address questions of representational alignment that stem from the following central theme: When and why 
  do intelligence systems learn aligned representations, and how can scientists and engineers intervene on this alignment? 
  Other questions topical for this year’s workshop include:
  - To what extent does representational alignment indicate shared computational strategies among biological and artificial systems?
  - How have current alignment metrics advanced our understanding of computation, and what measurement approaches should we explore next?
  - How can we develop more robust and generalizable measures of alignment that work across different domains and types of representations?
  - How can we systematically increase (or decrease) representational alignment among biological and artificial systems?
  - What are the implications (positive and negative) of increasing or decreasing representational alignment between systems, on behavioral alignment, value alignment, and beyond?

  **Second, by participating in our workshop [hackathon](#hackathon)**.
  Since the first iteration of Re-Align workshop, there have been numerous debates around the metrics that
  we use to measure representational similarity, which is often taken as a measure of representational alignment (e.g.,
  [Cloos et al., 2024](https://arxiv.org/abs/2407.07059); 
  [Khosla et al., 2024](https://doi.org/10.1101/2024.06.20.599957); 
  [Lampinen et al., 2024](https://arxiv.org/abs/2405.05847); 
  [Schaeffer et al., 2024](https://openreview.net/forum?id=vbtj05J68r)). 
  As of now, there is little consensus on which metric best achieves the goal of identifying similarity
  between systems.
  The hackathon component of the workshop will be helpful in articulating the consequences of these methodologies
  by facilitating a common language among researchers, and as a result increase the reproducibility of research in this
  subdomain.

hackathon: |
  *Details coming soon!*

program_committee:
  - url: https://openreview.net/profile?id=~Akash_Nagaraj2
    name: Akash Nagaraj
  - url: https://openreview.net/profile?id=~Alex_H_Williams1
    name: Alex H Williams
  - url: https://openreview.net/profile?id=~Alexander_F_Spies1
    name: Alexander F Spies
  - url: https://openreview.net/profile?id=~Alexander_Ku1
    name: Alexander Ku
  - url: https://openreview.net/profile?id=~Amit_Kumar_Jaiswal1
    name: Amit Kumar Jaiswal
  - url: https://openreview.net/profile?id=~Andrew_Kyle_Lampinen1
    name: Andrew Kyle Lampinen
  - url: https://openreview.net/profile?id=~Andrew_Shin1
    name: Andrew Shin
  - url: https://openreview.net/profile?id=~Aniket_Vashishtha1
    name: Aniket Vashishtha
  - url: https://openreview.net/profile?id=~Anil_B_Murthy1
    name: Anil B Murthy
  - url: https://openreview.net/profile?id=~Ann-Kathrin_Dombrowski1
    name: Ann-Kathrin Dombrowski
  - url: https://openreview.net/profile?id=~Anna_Bair1
    name: Anna Bair
  - url: https://openreview.net/profile?id=~Azmine_Toushik_Wasi1
    name: Azmine Toushik Wasi
  - url: https://openreview.net/profile?id=~Ben_Lonnqvist1
    name: Ben Lonnqvist
  - url: https://openreview.net/profile?id=~Bibek_Upadhayay1
    name: Bibek Upadhayay
  - url: https://openreview.net/profile?id=~Bo_Zhao6
    name: Bo Zhao
  - url: https://openreview.net/profile?id=~Bowen_Jiang2
    name: Bowen Jiang
  - url: https://openreview.net/profile?id=~Brian_R_Bartoldson1
    name: Brian R. Bartoldson
  - url: https://openreview.net/profile?id=~Cesar_Torrico1
    name: Cesar Torrico
  - url: https://openreview.net/profile?id=~Ching_Fang2
    name: Ching Fang
  - url: https://openreview.net/profile?id=~Chris_Lin1
    name: Chris Lin
  - url: https://openreview.net/profile?id=~Corrado_Rainone1
    name: Corrado Rainone
  - url: https://openreview.net/profile?id=~David_Lipshutz1
    name: David Lipshutz
  - url: https://openreview.net/profile?id=~Declan_Iain_Campbell1
    name: Declan Iain Campbell
  - url: https://openreview.net/profile?id=~Dongkyum_Kim1
    name: Dong-Kyum Kim
  - url: https://openreview.net/profile?id=~Dongping_Chen1
    name: Dongping Chen
  - url: https://openreview.net/profile?id=~Dongyu_Gong1
    name: Dongyu Gong
  - url: https://openreview.net/profile?id=~Eeshan_Hasan1
    name: Eeshan Hasan
  - url: https://openreview.net/profile?id=~Eghbal_A._Hosseini1
    name: Eghbal A. Hosseini
  - url: https://openreview.net/profile?id=~Elom_Amematsro1
    name: Elom Amematsro
  - url: https://openreview.net/profile?id=~Eloy_Geenjaar1
    name: Eloy Geenjaar
  - url: https://openreview.net/profile?id=~Emanuele_Marconato1
    name: Emanuele Marconato
  - url: https://openreview.net/profile?id=~Farzaneh_Taleb1
    name: Farzaneh Taleb
  - url: https://openreview.net/profile?id=~Fei_Ni1
    name: Fei Ni
  - url: https://openreview.net/profile?id=~Galen_Pogoncheff1
    name: Galen Pogoncheff
  - url: https://openreview.net/profile?id=~Greta_Tuckute1
    name: Greta Tuckute
  - url: https://openreview.net/profile?id=~Gustaw_Opielka1
    name: Gustaw Opielka
  - url: https://openreview.net/profile?id=~Haofei_Yu1
    name: Haofei Yu
  - url: https://openreview.net/profile?id=~Haoran_Zhao2
    name: Haoran Zhao
  - url: https://openreview.net/profile?id=~Hongkun_Hao1
    name: Hongkun Hao
  - url: https://openreview.net/profile?id=~Hyewon_Jeong1
    name: Hyewon Jeong
  - url: https://openreview.net/profile?id=~Iris_Groen1
    name: Iris Groen
  - url: https://openreview.net/profile?id=~Isabel_Papadimitriou1
    name: Isabel Papadimitriou
  - url: https://openreview.net/profile?id=~Ivan_F_Rodriguez_Rodriguez1
    name: Ivan F Rodriguez Rodriguez
  - url: https://openreview.net/profile?id=~Jacob_A_Zavatone-Veth1
    name: Jacob A Zavatone-Veth
  - url: https://openreview.net/profile?id=~Jacob_Granley1
    name: Jacob Granley
  - url: https://openreview.net/profile?id=~Jaekyeom_Kim1
    name: Jaekyeom Kim
  - url: https://openreview.net/profile?id=~Jaewon_Yoo1
    name: Jaewon Yoo
  - url: https://openreview.net/profile?id=~Jea_Kwon1
    name: Jea Kwon
  - url: https://openreview.net/profile?id=~Jenelle_Feather1
    name: Jenelle Feather
  - url: https://openreview.net/profile?id=~Jerry_Ngo1
    name: Jerry Ngo
  - url: https://openreview.net/profile?id=~Jiahong_Liu1
    name: Jiahong Liu
  - url: https://openreview.net/profile?id=~Jian-Qiao_Zhu1
    name: Jian-Qiao Zhu
  - url: https://openreview.net/profile?id=~Jinyung_Hong1
    name: Jinyung Hong
  - url: https://openreview.net/profile?id=~Jonas_Dippel1
    name: Jonas Dippel
  - url: https://openreview.net/profile?id=~Julien_Colin2
    name: Julien Colin
  - url: https://openreview.net/profile?id=~K._Seeliger1
    name: K. Seeliger
  - url: https://openreview.net/profile?id=~Kaiwen_Sheng1
    name: Kaiwen Sheng
  - url: https://openreview.net/profile?id=~Kevin_Matthew_Stangl1
    name: Kevin Stangl
  - url: https://openreview.net/profile?id=~Kijung_Yoon2
    name: Kijung Yoon
  - url: https://openreview.net/profile?id=~Kira_Michaela_Düsterwald1
    name: Kira Michaela Düsterwald
  - url: https://openreview.net/profile?id=~Lars_Kai_Hansen1
    name: Lars Kai Hansen
  - url: https://openreview.net/profile?id=~Lenka_Tětková1
    name: Lenka Tětková
  - url: https://openreview.net/profile?id=~Loek_van_Rossem1
    name: Loek van Rossem
  - url: https://openreview.net/profile?id=~Lorenz_Linhardt2
    name: Lorenz Linhardt
  - url: https://openreview.net/profile?id=~Luca_Maximilian_Schulze_Buschoff1
    name: Luca M. Schulze Buschoff
  - url: https://openreview.net/profile?id=~M_SAIFUL_BARI1
    name: M Saiful Bari
  - url: https://openreview.net/profile?id=~Marta_Kryven1
    name: Marta Kryven
  - url: https://openreview.net/profile?id=~Mathias_Sablé-Meyer1
    name: Mathias Sablé-Meyer
  - url: https://openreview.net/profile?id=~Max_Klabunde1
    name: Max Klabunde
  - url: https://openreview.net/profile?id=~Mayank_Kejriwal1
    name: Mayank Kejriwal
  - url: https://openreview.net/profile?id=~Meng_Du1
    name: Meng Du
  - url: https://openreview.net/profile?id=~Michael_A._Lepori1
    name: Michael A. Lepori
  - url: https://openreview.net/profile?id=~Michael_Hanna1
    name: Michael Hanna
  - url: https://openreview.net/profile?id=~Mingmeng_GENG1
    name: Mingmeng Geng
  - url: https://openreview.net/profile?id=~Mitja_Nikolaus1
    name: Mitja Nikolaus
  - url: https://openreview.net/profile?id=~Moritz_Willig1
    name: Moritz Willig
  - url: https://openreview.net/profile?id=~Nanda_Harishankar_Krishna1
    name: Nanda H Krishna
  - url: https://openreview.net/profile?id=~Neehar_Kondapaneni1
    name: Neehar Kondapaneni
  - url: https://openreview.net/profile?id=~Neel_Jain1
    name: Neel Jain
  - url: https://openreview.net/profile?id=~Negin_Raoof1
    name: Negin Raoof
  - url: https://openreview.net/profile?id=~Nhut_Truong1
    name: Nhut Truong
  - url: https://openreview.net/profile?id=~Niharika_Shimona_Dsouza1
    name: Niharika S. D'Souza
  - url: https://openreview.net/profile?id=~Pablo_Hernández-Cámara1
    name: Pablo Hernández-Cámara
  - url: https://openreview.net/profile?id=~Paul_Steven_Scotti1
    name: Paul Steven Scotti
  - url: https://openreview.net/profile?id=~Peiyang_Song1
    name: Peiyang Song
  - url: https://openreview.net/profile?id=~Peng_Qian1
    name: Peng Qian
  - url: https://openreview.net/profile?id=~Pengrui_Han1
    name: Pengrui Han
  - url: https://openreview.net/profile?id=~Peter_G._Chang1
    name: Peter G. Chang
  - url: https://openreview.net/profile?id=~Rizwan_Qureshi1
    name: Rizwan Qureshi
  - url: https://openreview.net/profile?id=~Robert_Geirhos1
    name: Robert Geirhos
  - url: https://openreview.net/profile?id=~Rohini_Elora_Das2
    name: Rohini Elora Das
  - url: https://openreview.net/profile?id=~Ruchira_Dhar1
    name: Ruchira Dhar
  - url: https://openreview.net/profile?id=~Rylan_Schaeffer2
    name: Rylan Schaeffer
  - url: https://openreview.net/profile?id=~Sameera_Horawalavithana1
    name: Sameera Horawalavithana
  - url: https://openreview.net/profile?id=~Samuel_Lippl1
    name: Samuel Lippl
  - url: https://openreview.net/profile?id=~Sarath_Sivaprasad2
    name: Sarath Sivaprasad
  - url: https://openreview.net/profile?id=~Sean_Michael_Kennedy1
    name: Sean M Kennedy
  - url: https://openreview.net/profile?id=~Shao-Qun_Zhang1
    name: Shao-Qun Zhang
  - url: https://openreview.net/profile?id=~Sherol_Chen1
    name: Sherol Chen
  - url: https://openreview.net/profile?id=~Liu_Shudong1
    name: Shudong Liu
  - url: https://openreview.net/profile?id=~Stephanie_Fu1
    name: Stephanie Fu
  - url: https://openreview.net/profile?id=~Sushrut_Thorat1
    name: Sushrut Thorat
  - url: https://openreview.net/profile?id=~Thomas_Anderson_Keller1
    name: T. Anderson Keller
  - url: https://openreview.net/profile?id=~Tahereh_Toosi2
    name: Tahereh Toosi
  - url: https://openreview.net/profile?id=~Tal_Golan1
    name: Tal Golan
  - url: https://openreview.net/profile?id=~Tanwi_Mallick1
    name: Tanwi Mallick
  - url: https://openreview.net/profile?id=~Thomas_Edward_Yerxa1
    name: Thomas Edward Yerxa
  - url: https://openreview.net/profile?id=~Thomas_L._Botch1
    name: Thomas L. Botch
  - url: https://openreview.net/profile?id=~Thomas_Miconi1
    name: Thomas Miconi
  - url: https://openreview.net/profile?id=~Thomas_S._A._Wallis1
    name: Thomas S. A. Wallis
  - url: https://openreview.net/profile?id=~Tianyu_Du1
    name: Tianyu Du
  - url: https://openreview.net/profile?id=~Tikeng_Notsawo_Pascal1
    name: Tikeng Notsawo Pascal Junior
  - url: https://openreview.net/profile?id=~Tom_Kouwenhoven1
    name: Tom Kouwenhoven
  - url: https://openreview.net/profile?id=~Wei-Chun_Huang1
    name: Wei-Chun Huang
  - url: https://openreview.net/profile?id=~Xiaoliang_Luo1
    name: Xiaoliang Luo
  - url: https://openreview.net/profile?id=~Xiaomeng_Wang7
    name: Xiaomeng Wang
  - url: https://openreview.net/profile?id=~Xiaoxuan_Lei1
    name: Xiaoxuan Lei
  - url: https://openreview.net/profile?id=~Xizheng_Yu1
    name: Xizheng Yu
  - url: https://openreview.net/profile?id=~Yan-Ying_Chen1
    name: Yan-Ying Chen
  - url: https://openreview.net/profile?id=~Yivan_Zhang1
    name: Yivan Zhang
  - url: https://openreview.net/profile?id=~Yu_Wang24
    name: Yu Wang
  - url: https://openreview.net/profile?id=~yudilin1
    name: Yudi Lin
  - url: https://openreview.net/profile?id=~Zeeshan_Memon2
    name: Zeeshan Memon
  - url: https://openreview.net/profile?id=~Zhehang_Du1
    name: Zhehang Du
  - url: https://openreview.net/profile?id=~Ziqi_Wen2
    name: Ziqi Wen
  - url: https://openreview.net/profile?id=~Zitong_Lu2
    name: Zitong Lu
---
